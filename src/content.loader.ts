 ...
 * This function reads from scraped/products.jsonl and creates individual JSON files
 * organized by language in src/content/products/{lang}/
 */
export async function loadScrapedProducts(): Promise<void> {
  ...
  for (const [lang, products] of productsByLang) {
    console.debug(`Processing language: ${lang}, productsBasePath: ${productsBasePath}`);
    const langPath = path.join(productsBasePath, lang);
 ...
 * This function reads from scraped/categories.jsonl for reference
 */
export async function loadScrapedCategories(): Promise<Array<{
  ...
  const scrapedCategoriesPath = path.resolve(projectRoot, 'scraped', 'categories.jsonl');
 ...
 * This function reads from scraped/categories.jsonl and creates individual JSON files
 * organized by language in src/content/categories/{lang}/
...
export async function loadCategoriesToContent(): Promise<void> {
  ...
  if (availableLanguages.size === 0) {
    console.warn('No languages found in scraped products');
    return;
  }
  ...
  for (const lang of availableLanguages) {
    const categoriesDictPath = path.join(dictionariesPath, lang, 'categories.json');
  ...
  for (const lang of availableLanguages) {
    const langPath = path.join(categoriesBasePath, lang);
